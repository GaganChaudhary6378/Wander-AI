#!/usr/bin/env bash
# Fetch app env vars from AWS SSM Parameter Store (path /ai-league/app/) and write ~/.env.
...
if [[ -z "${AWS_DEFAULT_REGION:-}" && -z "${AWS_REGION:-}" ]]; then
  export AWS_DEFAULT_REGION=$(curl -s -m 2 http://169.254.169.254/latest/dynamic/instance-identity/document | grep -o '"region" : "[^"]*"' | cut -d'"' -f4) || true
fi
...
# Write KEY=VALUE (strip path prefix from Name; Name is e.g. /ai-league/app/OPENAI_API_KEY)
